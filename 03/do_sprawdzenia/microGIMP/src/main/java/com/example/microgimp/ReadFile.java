...
public class ReadFile {
    ...
    private static float[][] readP4(int rows, int columns, String filepath) throws IOException {
        try (FileInputStream fis = new FileInputStream(filepath);
            ...
            byte[] buffer = new byte[rows * ((columns / 8) + 1)];
            bis.read(buffer, 0, 3); // Skip header
            bis.read(buffer, 0, rows * ((columns / 8) + 1)); // Read data

            float[][] bits = new float[rows][columns * 3];
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < columns; j++) {
                    int next = buffer[(i * ((columns / 8) + 1)) + (j / 8)] & (1 << (7 - (j % 8)));
                    bits[i][j * 3] = next > 0 ? 1.0f : 0.0f;
                    bits[i][j * 3 + 1] = next > 0 ? 1.0f : 0.0f;
                    bits[i][j * 3 + 2] = next > 0 ? 1.0f : 0.0f;
                }
            }
            return bits;
        }
