...
public class DisplayController implements Initializable{
    ...
    @FXML
    private Button previtButton;
    @FXML
    private Button robertsButton;
    @FXML
    private Button laplaceButton;
    @FXML
    private Button logFilterButton;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        loadImgButton.setEffect(dropShadow);
        saveImgButton.setEffect(dropShadow);
        negativeButton.setEffect(dropShadow);
        sumButton.setEffect(dropShadow);
        subtractionButton.setEffect(dropShadow);
        productButton.setEffect(dropShadow);
        revertChanges.setEffect(dropShadow);
        stretchingButton.setEffect(dropShadow);
        equalizationButton.setEffect(dropShadow);
        stdBlurButton.setEffect(dropShadow);
        gaussianBlurButton.setEffect(dropShadow);
        sharpenImageButton.setEffect(dropShadow);
        sobelButton.setEffect(dropShadow);
        previtButton.setEffect(dropShadow);
        robertsButton.setEffect(dropShadow);
        laplaceButton.setEffect(dropShadow);
        logFilterButton.setEffect(dropShadow);

        displayedImage.setFitHeight(700);
    ...
    public void loadMainFile(ActionEvent event) throws IOException {
        bits = ReadFile.main();
        originalBits = bits;
        update(event);
    }
    ...
    public void update(ActionEvent event) {
        ...
        wImage = new WritableImage(bits[0].length, bits.length);
    ...
    public void sumaObr(ActionEvent event) throws IOException {
        bits2 = ReadFile.main();
        bits = ImageOperations.imageSum(bits.length, bits[0].length, bits, bits2);
        update(event);
    }

    public void roznObr(ActionEvent event) throws IOException {
        bits2 = ReadFile.main();
        bits = ImageOperations.imageSubtraction(bits.length, bits[0].length, bits, bits2);
        update(event);
    }

    public void iloObr(ActionEvent event) throws IOException {
        bits2 = ReadFile.main();
        bits = ImageOperations.imageProduct(bits.length, bits[0].length, bits, bits2);
        update(event);
    }

    public void standardBlur(ActionEvent event){
        Filters.standardBlur();
    }


    public void gaussianBlur(ActionEvent event){
        Filters.gaussianBlur();
    }
    public void imageSharpening(ActionEvent event){
        Filters.imageSharpening();
    }
    public void sobelFiltering(ActionEvent event){
        Filters.sobelFiltering();
    }
    public void previtFilter(ActionEvent event){
        Filters.previtFilter();
    }
    public void robertsFiltering(ActionEvent event){
        Filters.robertsFiltering();
    }
    public void laplaceFiltering(ActionEvent event){
        Filters.laplaceFiltering();
    }
    public void logFiltering(ActionEvent event){
        Filters.logFiltering();
    }
