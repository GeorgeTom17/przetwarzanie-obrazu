...
public class DisplayController implements Initializable{
    ...
    @FXML
    private Slider watershedThreshold;
    @FXML
    private Label watershedThresholdVal;
    ...
    @FXML
    private Slider harrisMinDist;
    @FXML
    private Label harrisMinDistVal;
    @FXML
    private Slider harrisThreshold;
    @FXML
    private Label harrisThresholdVal;

    @FXML
    private Slider splitAndMergeVariance;
    @FXML
    private Label splitAndMergeVarianceVal;
    @FXML
    private Slider splitAndMergeMinSize;
    @FXML
    private Label splitAndMergeMinSizeVal;
    @FXML
    private Slider splitAndMergeJoinThreshold;
    @FXML
    private Label splitAndMergeJoinThresholdVal;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        harrisMinDist.setValue(0);
        harrisMinDistVal.textProperty().bind(Bindings.format("%.0f", harrisMinDist.valueProperty()));
        harrisThreshold.setValue(0);
        harrisThresholdVal.textProperty().bind(Bindings.format("%.0f", harrisThreshold.valueProperty()));
        splitAndMergeVariance.setValue(0);
        splitAndMergeVarianceVal.textProperty().bind(Bindings.format("%.10f", splitAndMergeVariance.valueProperty()));
        splitAndMergeMinSize.setValue(0);
        splitAndMergeMinSizeVal.textProperty().bind(Bindings.format("%.0f", splitAndMergeMinSize.valueProperty()));
        splitAndMergeJoinThreshold.setValue(0);
        splitAndMergeJoinThresholdVal.textProperty().bind(Bindings.format("%.2f", splitAndMergeJoinThreshold.valueProperty()));
        watershedThreshold.setValue(0);
        watershedThresholdVal.textProperty().bind(Bindings.format("%.0f", watershedThreshold.valueProperty()));
        ...
        harrisMinDist.valueProperty().addListener((obs, oldVal, newVal) ->
                harrisMinDist.setValue(Math.round(newVal.doubleValue())));
        harrisThreshold.valueProperty().addListener((obs, oldVal, newVal) ->
                harrisThreshold.setValue(Math.round(newVal.doubleValue())));
        splitAndMergeMinSize.valueProperty().addListener((obs, oldVal, newVal) ->
                splitAndMergeMinSize.setValue(Math.round(newVal.doubleValue())));
        watershedThreshold.valueProperty().addListener((obs, oldVal, newVal) ->
                watershedThreshold.setValue(Math.round(newVal.doubleValue())));
        ...
        splitAndMergeVariance.valueProperty().addListener((observable) -> splitAndMerge());

        splitAndMergeVariance.valueChangingProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
                // User has stopped dragging the slider
                splitAndMerge();
            }
        ...
        splitAndMergeMinSize.valueProperty().addListener((observable) -> splitAndMerge());

        splitAndMergeMinSize.valueChangingProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
                // User has stopped dragging the slider
                splitAndMerge();
            }
        ...
        splitAndMergeJoinThreshold.valueProperty().addListener((observable) -> splitAndMerge());

        splitAndMergeJoinThreshold.valueChangingProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
                // User has stopped dragging the slider
                splitAndMerge();
            }
    ...
    public void negative() {
        ...
        updatePrevBits();*/
        bits = CornerDetection.detectCorner(prevBits, 9, 50);
        applyChanges.fire();
    }
    ...
    public void harrisCornerDetection(){
        bits = CornerDetection.detectCorner(prevBits, (int) (harrisMinDist.getValue()), (int) harrisThreshold.getValue());
        applyChanges.fire();
    }

    public void splitAndMerge(){
        bits = SplitAndMerge.splitAndMerge(prevBits, (float) splitAndMergeVariance.getValue(), (int) splitAndMergeMinSize.getValue(), (float) splitAndMergeJoinThreshold.getValue());
        applyChanges.fire();
    }

    public void findSplitAndMergeVar(){
        float[] minMaxVar= SplitAndMerge.findMinMaxVar(EdgeDetection.reduceMatrix(EdgeDetection.turnGrayscale(bits)));
        splitAndMergeVariance.setMin(minMaxVar[0]);
        splitAndMergeVariance.setMax(minMaxVar[1]);

    }

    public void watershed(){
        Watershed.watershedSegmentation(filepath, (int) watershedThreshold.getValue());
        applyChanges.fire();
    }
