...
public class Filters {
            ...
            for (int x = 0; x < width; x++) {
                ...
                for (int i = -starterPoint; i <= starterPoint; i++) {
                    for (int j = -starterPoint; j <= starterPoint; j++) {
                        ...
                        float[] neighbor = getPixel(image, posX, posY);
                        for (int k = 0; k < 3; k++) {
    ...
    public static float[] getPixelWithValidation(float[][] image, int x, int y) {
        int width = image[0].length / 3;
        int height = image.length;
    ...
    public static void setPixelWithValidation(float[][] image, int x, int y, float[] pixel) {
        int width = image[0].length / 3;
        int height = image.length;
        if (x < 0 || x >= width || y < 0 || y >= height) {
    ...
    public static float[] getPixel(float[][] image, int x, int y) {
        float[] pixel = new float[3];
        System.arraycopy(image[y], x * 3, pixel, 0, 3);
        return pixel;
    }

    public static void setPixel(float[][] image, int x, int y, float[] pixel) {
